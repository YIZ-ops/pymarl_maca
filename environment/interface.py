...
class Environment:
    ...
    def __init__(self, map_path, side1_obs_ind, side2_obs_ind, max_step=5000, render=False, render_interval=1,
        ...
        if 'raw' == side1_obs_ind:
            ...
            self.agent1_state = self.agent1_obs_module.StateConstruct(self.size_x, self.size_y, self.side1_detector_num,
                                                        self.side1_fighter_num, self.side2_detector_num,
        ...
        if 'raw' == side2_obs_ind:
            ...
            self.agent2_obs = self.agent2_obs_module.ObsConstruct(self.size_x, self.size_y, self.side2_detector_num,
                                                                  self.side2_fighter_num)
            self.agent2_state = self.agent2_obs_module.StateConstruct(self.size_x, self.size_y, self.side2_detector_num,
                                                            self.side2_fighter_num, self.side1_detector_num,
                                                            self.side1_fighter_num)
    ...
    def get_state(self):
        """
        Get image-based state
        :return: side1_state
        :return: side2_state
        """
        side1_obs_raw_dict, side2_obs_raw_dict = self.get_obs_raw()
        if 'obs_construct.simple_copy.construct' == self.side1_obs_path:
            side1_state = self.agent1_state.state_construct(side1_obs_raw_dict, side2_obs_raw_dict)
        else:
            side1_state =  side1_obs_raw_dict
            
        if 'obs_construct.simple_copy.construct' == self.side2_obs_path:
            side2_state = self.agent2_state.state_construct(side2_obs_raw_dict, side1_obs_raw_dict)
        else:
            side2_state =  side2_obs_raw_dict

        return side1_state, side2_state
    ...
    def get_obs(self):
        ...
        elif 'obs_construct.simple_copy.construct' == self.side1_obs_path:
            side1_obs = self.agent1_obs.obs_construct(side1_obs_raw_dict, self.side2_detector_num, self.side2_fighter_num)
        else:
        ...
        if 'raw' == self.side2_obs_path:
            ...
            side2_obs = self.agent2_obs.obs_construct(side2_obs_raw_dict, self.side1_detector_num, self.side1_fighter_num)
        else:
